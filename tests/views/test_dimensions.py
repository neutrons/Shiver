...
def test_dimensions_radio_btn(qtbot):
    ...
    assert histogram_parameters.dimesions.steps_valid_state() == True
...
def test_dimensions_ste_values_required_color(qtbot):
    ...
    css_style_step1 = histogram_parameters.dimesions.combo_step1.styleSheet()
    css_style_step2 = histogram_parameters.dimesions.combo_step2.styleSheet()
    css_style_step3 = histogram_parameters.dimesions.combo_step3.styleSheet()
    css_style_step4 = histogram_parameters.dimesions.combo_step4.styleSheet()
    ...
    assert histogram_parameters.dimesions.steps_valid_state() == False    
...
def test_dimensions_update_on_projection_edit(qtbot):    
    """Test for dimensions dropdown selection values are unique among them
        test case are created based on  example scenarions atMantidWorkBench -> Interfaces -> Direct -> DGSPlanner"""
    ...
    # dimensions 1-3
    assert histogram_parameters.dimesions.combo_dim1.currentText() == "[13.0H,H,0]"
    assert histogram_parameters.dimesions.combo_dim2.currentText() == "[K,8.0K,0]"
    assert histogram_parameters.dimesions.combo_dim3.currentText() == "[L,L,6.0L]"    
    
    #test 2
    #assign 4 = [0]
    qtbot.keyClicks(histogram_parameters.dimesions.combo_dim4, "[13.0H,H,0]")     
    
    # dimensions 1-4
    assert histogram_parameters.dimesions.combo_dim1.currentText() == "DeltaE"
    assert histogram_parameters.dimesions.combo_dim2.currentText() == "[K,8.0K,0]"
    assert histogram_parameters.dimesions.combo_dim3.currentText() == "[L,L,6.0L]"  
    assert histogram_parameters.dimesions.combo_dim4.currentText() == "[13.0H,H,0]"        
    ...
    # dimensions 1-4
    assert histogram_parameters.dimesions.combo_dim1.currentText() == "[6.0H,6.0H,6.0H]"
    assert histogram_parameters.dimesions.combo_dim2.currentText() == "[2.0L,L,3.0L]"
    assert histogram_parameters.dimesions.combo_dim3.currentText() == "[0,0,L]"  
    assert histogram_parameters.dimesions.combo_dim4.currentText() == "DeltaE"     
